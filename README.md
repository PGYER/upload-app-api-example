...
Java、C#、PHP、Shell 示例仅对 `buildInfo` 的 `1246`、`1247`（处理中）继续轮询；其他非零业务码立即停止并显示原始错误码与错误说明。`0` 表示成功。终态错误（例如 `1249`）需要根据错误说明处理，不能通过继续轮询解决。

这些示例仍最多轮询 60 次；请求耗时不包含在每次 1 秒的等待间隔内，因此不是严格的 60 秒总超时。PHP 在持续处理中耗尽次数时保留原来的 `false` 返回行为。

离线验证方法见 [轮询回归检查](regression/README.md)。
